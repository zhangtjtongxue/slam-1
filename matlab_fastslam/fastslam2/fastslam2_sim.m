...
%function [data, history]= fastslam2_sim(lm, wp)
%
% INPUTS: 
%   lm - set of landmarks
%   wp - set of waypoints
...
%%%%%%%%%%%%%%%%%%% BEGIN OF CHANGES %%%%%%%%%%%%%%%%%%%
% Transpose incoming landmark matrix that is coming from python
lm= lm.';

% The maximum amount of control inputs is now the limitation
max_u= columns(wp);

% Keep all predictions:
history= zeros(3,max_u);

%if exist('OCTAVE_VERSION') ~= 0
%	% you'll need to run this once, to install the package:
%	% pkg install -forge parallel
%  disp("Load the parallel package");
%	pkg load parallel
%  
%  numCores = nproc();
%end
...
%%%%%%%%%%%%% COMMENTED OUT TO DISABLE GRAPHICS & ANIMATION %%%%%%%%%%%%%
...
% Main loop 
%%%%%%%%%%%%% CHANGE PARAMETER TO LOOP OVER ALL CONTROL INPUTS %%%%%%%%%%%%%
while iwp <= max_u 
    ...
    % Instead of computing the path to the next waypoint, simply choose
    %   the next heading given from our list --> this lets us run the
    %   the path from our python simulation.
    ...
    % We just want to do one run for now. Don't need this then.
    ...
    for i=1:NPARTICLES
        particles(i).true= [particles(i).true,xtrue];;
    end
 
 
    %%%%%%%%%%%%%%%%%%% BEGIN OF CHANGES %%%%%%%%%%%%%%%%%%%
    %
    % Modification: In original, error was added directly to controls [V,G]
    % In our work, the noise gets added after moving. 
    % Therefore: Move noise computation to predict function and comment following line.
    %
    % add process noise
    ...
    if dtsum >= DT_OBSERVE
        ...
        for i=1:NPARTICLES
          for j=ftag_visible, if !ismember(j,particles(i).asso), particles(i).asso= [particles(i).asso,j];end,endfor;
        end
        
        % for i=b, if !ismember(i,a), a=[a,i], end, endfor
    ...
    %%%%%%%%%%%%%%%%%%% BEGIN OF CHANGES %%%%%%%%%%%%%%%%%%%
    pos_pred= zeros(3,NPARTICLES);
    wghts = zeros(1,NPARTICLES);
    % print current prediction according to weights
    for i=1:NPARTICLES
      pos_pred(:,i)= particles(i).xv;
      wghts(:,i)= particles(i).w;
    end
    current_position_prediction = mean(pos_pred*(wghts.'),2);
    history(:, iwp)= current_position_prediction;
...
data= particles;
dlmwrite('../mats/prediction_history.mat',history.','delimiter','\t','precision',3)
% 
%
...
function p= initialise_particles(np,truestate)
for i=1:np
    p(i).w= 1/np;
    % p(i).xv= [0;0;0]; 
    p(i).xv= truestate;
